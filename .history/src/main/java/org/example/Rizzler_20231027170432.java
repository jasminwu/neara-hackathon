...
public class Rizzler extends Bot {
    ...
    private boolean willCollide(int width, int height) {
        ...
        if (goingUp(dir)) {
            ...
            double relevantY = height;
            double extraX = distanceTo(x, height) / Math.cos(angle);
            double relevantX = (90 + 180 - dir > 0) ? x - extraX : x + extraX;

            return distanceTo(relevantX, relevantY) < 18;
        } else {
            // Calculate distance to closest point on right wall

            double angle = Math.min(dir, 360 - dir);
            double relevantX = width;
            double extraY = distanceTo(width, y) / Math.cos(angle);

            double relevantY = dir < 45 ? y - extraY : y - extraY;
